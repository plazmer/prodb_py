...
def func01(connection):
    with open('02.csv') as file:
        ...
        cursor.execute('DROP TABLE IF EXISTS func01')
        cursor.execute('''CREATE TABLE IF NOT EXISTS func01 (
                                      id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, 
        ...
        connection.commit()
        cursor.executemany('''INSERT INTO func01
                                (address,birthdate,blood_group,company,job,mail,name,phone,residence,sex,ssn,username,website)     
                                VALUES (:address,:birthdate,:blood_group,:company,:job,:mail,:name,:phone,:residence,:sex,:ssn,:username,:website)''', data_list)
        connection.commit()
        cursor.execute('SELECT COUNT(*) as cnt FROM func01')
...
def func02(phone,connection):
    cursor = connection.cursor()
    cursor.execute('SELECT name FROM func01 WHERE phone=:phone', (phone,))
