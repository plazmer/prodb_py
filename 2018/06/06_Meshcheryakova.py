...
class SqliteDb:
    ...
    def __init__(self, db_file):
        if db_file:
            self.default_db = '../06_Meshcheryakova.db'
    ...
    def rand(self):
        ...
        return cache.get_by(dict(self.cursor.fetchone())['publish_date'])
...
class NoSQLCache:
    ...
    def get(self, key):
        ...
        if value:
            ...
            self.set(key, value, self.timeout)
            return value

    def set(self, key, value, timeout):
        self.client.set(key, value, px=timeout)
...
def bydate(date):
    cache = NoSQLCache.inst()
    j = json.dumps(cache.get_by(date))
    return j
