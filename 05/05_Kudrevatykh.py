...
app = Bottle()
# r = Redis() #TODO
db = MongoClient().s_database
counters = db.counters
if not counters.find_one({"_id": 1}):
    counters.insert_one({"_id": 1, "visitor": 0})
data = db.data
...
def index():
    #todo
    counters.update_one({"_id": 1}, {"$inc": {"visitor": 1}})
    counter = counters.find_one({"_id": 1})["visitor"]
    #/todo
...
def list_keys():
    ...
    for key in keys_list:
        response += '''<div class="list_item"><a href="/get/%s">Get key %s</a><br /> <a href="/del/%s">Delete key %s</a></div>'''%(key,key,key,key)
    return  template('% rebase("static/index.html")\n'+response)
