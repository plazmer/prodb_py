...
def index():
    if not counters.find_one({"_id": 1}):
        counters.insert_one({"_id": 1, "Vs": 0})
    counter = counters.find_one_and_update({"_id": 1}, {"$inc": {"Vs": 1}}, return_document=ReturnDocument.AFTER)["Vs"]
    vars = {'counter':counter, 'header':'Tom Hanks is the Best', 'footer':'Run, Forest, Run'}
