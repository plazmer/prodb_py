...
def func01(connection):
    with open ('02.csv') as f:
        f1 = csv.DictReader(f)
        to_f1 = [(row['address'], row['birthdate'], row['blood_group'], row['company'], row['job'], row['mail'], row['name'], row['phone'], row['residence'], row['sex'], row['ssn'], row['username'], row['website']) for row in f1] 
        c = connection.cursor()
        ...
        c.executemany('''INSERT INTO func01 
...
def func02(phone, connection):
    c = connection.cursor()
    for res in c.execute('SELECT name FROM func01 WHERE phone=?', (phone,)):
        return res[0]# it's magic, но работает
    c.close()
