...
def func00(words):
    count = [w for w in words if len(w)>=2 and w[0]==w[-1]]
    return len(count)
...
def func01(words): #не оптимально, но компактно
    return sorted([x for x in words if ord(x[0])>57]) + sorted([x for x in words if x[0].isdigit()])


# 02. Отсортировать по последнему
# Дан список не пустых tuples, вернуть список, отсортированный по возрастанию последнего элемента tuple
def func02(tuples):    
    return sorted(tuples, key = lambda i: i[-1]) #больше и добавить нечего
